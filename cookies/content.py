...
def handle_content(obj, commit=True):
    ...
    """
    if indexable(obj.content_type):
        if obj.content_type == 'text/plain':
            obj.indexable_content = obj.file.read()
        elif obj.content_type in pdf_mime_types:
            obj.indexable_content = pdf_extract(obj.file)
        elif obj.content_type in xml_mime_types:
            obj.indexable_content = soup_extract(obj.file)
        else:
