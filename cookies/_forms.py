...
class LocalResourceForm(ResourceForm):
    class Meta:
        model = LocalResource
        fields = ('name', 'file', 'content_type', 'entity_type', 'uri',
                  'public')
...
class RelationForm(forms.ModelForm):
    ...
    target = TargetField(queryset=Entity.objects.all(),
    ...
    def clean(self):
        ...
        if len(target_data) == 0:
            self._errors['target'] = self.error_class(['Cannot be blank.'])
        ...
        #  predicate Field's range.
            ...
            #  a StringValue.
                ...
                try:
                    target_obj = Entity.objects.get(name=unicode(target_data)).cast()
                except ObjectDoesNotExist:
                    target_obj = StringValue()
                    target_obj.name = unicode(target_data)
                    target_obj.save()
                cast = True
